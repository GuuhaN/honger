...
const SuggestionModal = dynamic(() => import("../../components/modals/suggestionModal"), { ssr: false });
...
export default function App() {
  ...
  const [randomNumber, setRandomNumber] = useState<number>(Math.max() * Math.random());
  ...
  const rateRestaurant = (score: number) => {
    ...
    createRating({
      ...
      if (!response) return;
      alert(`Jij hebt ${restaurants[randomNumber].name} een ${score} hongurating gegeven.`);
  ...
  return (
    <div className="flex bg-green-100 h-screen">
      <Snowfall />
      {restaurants && restaurants[randomNumber]?.name.replace(/\s/g, "").toLowerCase().match("kfc") && (
        <>
          <Confetti
      ...
      <div className="flex flex-col justify-center m-auto gap-4">
        <div className="flex flex-col self-center gap-4">
          <div className="flex flex-col gap-2">
            <Typography type="h1" >Honger {"<°>"}</Typography>
            <Typography type="h2">Eten</Typography>
            <Typography type="h3">Schuiven</Typography>
            <div className="flex">
              <Button
                variant="Primary"
                type="submit"
                onClick={() => {
                  ...
                  if (restaurants) {
                    getRestaurantRatings(restaurants[randNum].id).then((response) => {
                      setRestaurantRating(response);
                    });
                  }
                  setRating(false);
        ...
        <div className="self-center text-center gap-2 flex flex-col">
          ...
          <div className="text-2xl tracking-widest">
            {restaurants && restaurants[randomNumber]
              ? `${restaurants[randomNumber].name} (${
                  restaurantRating?.ratings && restaurantRating?.ratings.length > 0
          ...
          {restaurants && restaurants[randomNumber] && (
            <div className="flex flex-col gap-2">
              <div className="text-xl tracking-widest">{restaurants[randomNumber].address}</div>
